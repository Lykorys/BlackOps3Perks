...
namespace BoneTest.Content.Items.Consumables
{
	...
	public class TestBossSummonItem : ModItem
	{
		...
		public override bool CanUseItem(Player player) {
			...
			return !NPC.AnyNPCs(ModContent.NPCType<TestBoss>());
		}
		public override void ModifyTooltips(List<TooltipLine> tooltips) {
            tooltips.Add(new TooltipLine(Mod,"test","Summons the master of puppets")
            {
                OverrideColor = new Color(142, 8, 39)
            });
		}
		public override bool? UseItem(Player player) {
			if (player.whoAmI == Main.myPlayer) {
				...
				int type = ModContent.NPCType<TestBoss>();

				if (Main.netMode != NetmodeID.MultiplayerClient) {
					// If the player is not in multiplayer, spawn directly
					NPC.NewNPC(NPC.GetSource_NaturalSpawn(), (int)player.Center.X, (int)player.Center.Y - 800, type);
				}
				else {
